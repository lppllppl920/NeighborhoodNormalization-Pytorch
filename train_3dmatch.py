...
from datasets import Match3DDataset
import datasets
import utils
import models
import losses
...
def main():
    parser = argparse.ArgumentParser(
        description='Geometric Feature Learning on 3DMatch Dataset',
    ...
    if not args.continue_train or not args.load_trained_weights:
        log_root = Path(args.log_root) / "3DMatch_train_{:02d}_{:02d}_{:02d}_{:02d}_{:02d}".format(
            date.month,
            date.day,
            date.hour,
            date.minute,
        ...
        for idx in range(len(parents)):
            if "3DMatch_train" in str(parents[idx].name):
    ...
    logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%m/%d %H:%M:%S', handlers=[ch, fh])

    train_mesh_model_path_list = sorted(list((Path(args.data_root) / "train").glob("*/seq*/cloud_bin*.ply")))
    val_mesh_model_path_list = sorted(list((Path(args.data_root) / "val").glob("*/seq*/cloud_bin*.ply")))

    train_dataset = Match3DDataset(
        phase="train",
        ...
        mesh_model_path_list=train_mesh_model_path_list,
        max_select_trial=args.max_sampling_trial,
        oversampling_factor=args.oversampling_factor,
        allow_repeat_sampling=args.allow_repeat_sampling,
        ...
        min_crop_remained_portion=None if args.crop_ratio_range is None else args.crop_ratio_range[0],
    )

    val_dataset = Match3DDataset(
        phase="eval",
        ...
        default_edge_length=args.default_edge_length,
        edge_length_range=args.edge_length_range,
        mesh_model_path_list=val_mesh_model_path_list,
        max_select_trial=args.max_sampling_trial,
        oversampling_factor=args.oversampling_factor,
        allow_repeat_sampling=True,
        overlap_ratio_threshold=args.overlap_ratio_threshold,
        use_rotation=args.use_rotation,
        rotate_range=args.rotate_range,
        use_remesh=args.use_remesh,
    ...
    for cur_epoch in range(epoch, args.max_num_epochs + 1):
        # Set the seed based on cur_epoch for reproducibility
        ...
        if (cur_epoch + 1) % args.val_freq == 0:
            ...
            with torch.no_grad():
                log = validation.validation_3dmatch(model=model, loader=val_loader, epoch=cur_epoch,
                                                    loss_func=loss_func, writer=writer,
                                                    val_sampling_size=args.val_sampling_size,
                                                    config_dict=config_dict,
                                                    num_iter=args.val_num_iter,
                                                    device=args.device,
                                                    vis_mesh=args.vis_mesh, vis_mesh_freq=args.vis_mesh_freq)
