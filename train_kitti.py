...
from datasets import KITTINMPairDataset
import datasets
import utils
import models
import losses
...
def main():
    parser = argparse.ArgumentParser(
        description='Geometric Feature Learning on KITTI Dataset',
    ...
    if not args.continue_train or not args.load_trained_weights:
        log_root = Path(args.log_root) / "KITTI_train_{:02d}_{:02d}_{:02d}_{:02d}_{:02d}".format(
            date.month,
            date.day,
            date.hour,
            date.minute,
        ...
        for idx in range(len(parents)):
            if "KITTI_train" in str(parents[idx].name):
    ...
    train_dataset = KITTINMPairDataset(phase="train", use_rotation=args.use_rotation,
                                       use_scale=args.use_scale,
                                       manual_seed=False, config=args)

    args.allow_repeat_sampling = True
    val_dataset = KITTINMPairDataset(phase="val", use_rotation=args.use_rotation,
                                     use_scale=args.use_scale,
                                     manual_seed=False, config=args)
    ...
    for cur_epoch in range(epoch, args.max_num_epochs + 1):
        ...
        if not args.val_only:
            for curr_iter, input_dict in enumerate(train_loader):
                ...
                if len(mean_att_list) >= 1:
                    ...
                    writer.add_scalars('Train', {'mean_att': mean_att[0].item(),
                                                 'std_att': std_att[0].item()}, step)
                else:
                    tq.set_postfix(loss='avg: {:.3f}, cur: {:.3f}'.format(total_loss_meter.avg, loss.item()),
                                   scale_ratio='{:.3f}'.format(input_dict['scale_ratio'][0].item()))
                    writer.add_scalar('Train/loss', loss.item(), step)
        ...
        if (cur_epoch + 1) % args.val_freq == 0:
            ...
            with torch.no_grad():
                log = validation.validation_kitti(model=model, loader=val_loader, epoch=cur_epoch,
            ...
            gc.collect()
            torch.cuda.empty_cache()
            model_path_epoch = log_root / 'checkpoint_model_epoch_{:d}_loss.pt'.format(
                cur_epoch, log['loss'])
            utils.save_checkpoint(epoch=cur_epoch, step=step, model=model,
                                  config=args, path=model_path_epoch)
